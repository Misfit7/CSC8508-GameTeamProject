...
RobotObject::RobotObject(NavigationGrid* navGrid, Vector3 initialPosition) : GameObject()
{
	...
	stateMachine->AddTransition(new StateTransition(FollowPlayerState, MoveToTreeState,
		[&]() -> bool
		{
			if (cutting && TutorialGame::GetGame()->GetController()->ButtonPressed("L3")) {
				treeCounter = 5.0f;
				nextTree = true;
				stateID = 2;
				return true;
	...
	stateMachine->AddTransition(new StateTransition(FollowPlayerState, MoveToRockState,
		[&]() -> bool
		{
			if (digging && TutorialGame::GetGame()->GetController()->ButtonPressed("L3")) {
				rockCounter = 5.0f;
				nextRock = true;
				stateID = 3;
				return true;
	...
	stateMachine->AddTransition(new StateTransition(FollowPlayerState, IdleState,
		[&]() -> bool
		{
			if (TutorialGame::GetGame()->GetController()->ButtonPressed("R3")) {
				player = nullptr;
				cutting = false;
				digging = false;
				stateID = 0;
	...
	stateMachine->AddTransition(new StateTransition(MoveToTreeState, MoveToPlayerState,
		[&]() -> bool
		{
			bool flag = moveToPlayer || (TutorialGame::GetGame()->GetController()->ButtonPressed("L3"));
			moveToPlayer = false;
			if (flag) {
				stateID = 6;
			}
	...
	stateMachine->AddTransition(new StateTransition(CuttingState, MoveToPlayerState,
		[&]() -> bool
		{
			bool flag = TutorialGame::GetGame()->GetController()->ButtonPressed("L3");
			if (flag) {
				stateID = 6;
			}
			return flag;
	...
	stateMachine->AddTransition(new StateTransition(MoveToPlayerState, IdleState,
		[&]() -> bool
		{
			bool flag = foundPlayer || TutorialGame::GetGame()->GetController()->ButtonPressed("R3");
			foundPlayer = false;
			if (flag) {
				player = nullptr;
				cutting = false;
...
void RobotObject::FollowPlayer(float dt) {
	...
	if (GetGrid(currentPosition).position != GetGrid(playerPosition).position) {
		...
		if (nodes.size() > 1) {
			...
			Maths::Vector::Normalise(direction);
			direction.y = 0;
			UpdateOrientation(direction);
			GetPhysicsObject()->AddForce(direction * speed);
			if (TutorialGame::GetGame()->ShowInfo())Debug::DrawLine(nodes[1], nodes[0], Vector4(0, 1, 0, 1));
			nodes.clear();
		}
	}
}
	...
	if (TutorialGame::GetGame()->GetWorld().Raycast(r, closestCollision, true, this)) {
		...
		if (closest->GetTypeID() == 10086) {
			if (closestCollision.rayDistance < 5.0f) {
				...
				UpdateOrientation(direction);
				GetPhysicsObject()->AddForce(direction * speed);
				if (TutorialGame::GetGame()->ShowInfo())Debug::DrawLine(transform.GetPosition(), closest->GetTransform().GetPosition(), Vector4(1, 1, 0, 1));
			}
		}
		else {
			treeCut = true;
