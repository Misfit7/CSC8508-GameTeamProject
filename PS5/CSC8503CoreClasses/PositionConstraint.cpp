...
void PositionConstraint::UpdateConstraint(float dt) {
    ...
    float currentDistance = Maths::Vector::Length(relativePos);
    float offset = distance - currentDistance;

    if (abs(offset) > 0.0f) {
        Vector3 offsetDir = Maths::Vector::Normalise(relativePos);
        ...
        if (constraintMass > 0.0f) {
            ...
            float velocityDot = Maths::Vector::Dot(relativeVelocity, offsetDir);
