...
float NavigationGrid::Heuristic(GridNode* hNode, GridNode* endNode) const {
    return Maths::Vector::Length(hNode->position - endNode->position);
}

Vector3 NavigationGrid::FindNearestTree(const Vector3& position) {
    std::vector<float> fs;
    for (int i = 0; i < gridWidth * gridHeight; ++i) {
        if (allNodes[i].type == 10086 && Maths::Vector::Length(allNodes[i].position - position) < 43) {
            float f = 50000.0f;
...
Vector3 NavigationGrid::FindNearestRock(const Vector3& position) {
    std::vector<float> fs;
    for (int i = 0; i < gridWidth * gridHeight; ++i) {
        if (allNodes[i].type == 10010 && Maths::Vector::Length(allNodes[i].position - position) < 43) {
            float f = 50000.0f;
            FindPath(position, allNodes[i].position, f);
            fs.push_back(f);
        }
