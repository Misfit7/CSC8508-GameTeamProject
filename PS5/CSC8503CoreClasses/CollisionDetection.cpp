...
bool CollisionDetection::RayPlaneIntersection(const Ray&r, const Plane&p, RayCollision& collisions) {
	float ln = Maths::Vector::Dot(p.GetNormal(), r.GetDirection());
	...
	float d = Maths::Vector::Dot(pointDir, p.GetNormal()) / ln;
...
bool CollisionDetection::RayBoxIntersection(const Ray&r, const Vector3& boxPos, const Vector3& boxSize, RayCollision& collision) {
	...
	float bestT = Maths::Vector::GetMaxElement(tVals);
...
int CollisionDetection::RayCylinderIntersection(const Ray& r, const float halfHeight, const float radius, RayCollision& collision) {
	...
	if (tTop >= 0 && Maths::Vector::Length(xzTop) < radius) validTs.push_back(tTop);

	float tBottom = (hMin - rayPos.y) / rayDir.y;
	Vector2 xzBottom(rayPos.x + tBottom * rayDir.x, rayPos.z + tBottom * rayDir.z);
	if (tBottom >= 0 && Maths::Vector::Length(xzBottom) < radius) validTs.push_back(tBottom);
...
bool CollisionDetection::RaySphereIntersection(const Ray&r, const Transform& worldTransform, const SphereVolume& volume, RayCollision& collision) {
	...
	float sphereProj = Maths::Vector::Dot(dir, r.GetDirection());
	...
	float sphereDist = Maths::Vector::Length(point - spherePos);
...
bool CollisionDetection::SATTest(const Vector3& sizeA, const Matrix3& rotationA, const Vector3& sizeB, const Matrix3& rotationB,
	...
	overlap = 0.0f;
	float localDist = Maths::Vector::Dot(centerDist, axis);
	localDist = abs(localDist);
	float radiusA = 0.0f, radiusB = 0.0f;
	for (int i = 0; i < 3; i++) {
		radiusA += sizeA[i] * abs(Maths::Vector::Dot(rotationA.GetColumn(i), axis));
		radiusB += sizeB[i] * abs(Maths::Vector::Dot(rotationB.GetColumn(i), axis));
	}
...
bool CollisionDetection::SphereIntersection(const SphereVolume& volumeA, const Transform& worldTransformA,
	...
	float deltaLength = Maths::Vector::Length(delta);

	if (deltaLength < radii) {
		float penetration = (radii - deltaLength);
		Vector3 normal = Maths::Vector::Normalise(delta);
...
bool CollisionDetection::AABBSphereIntersection(const AABBVolume& volumeA, const Transform& worldTransformA,
	...
	//float distance = Maths::Vector::Length(localPoint);
	//
	//if (distance < volumeB.GetRadius()) {
	//	Vector3 collisionNormal = Maths::Vector::Normalise(localPoint);
...
//	const AABBVolume& volumeB, const Transform& worldTransformB, CollisionInfo& collisionInfo) {
	...
	//Vector3 boxClosest = Maths::Vector::Clamp(delta, -boxSize, boxSize);
	//Vector3 localP = delta - boxClosest;
	//float distance = Maths::Vector::Length(localP);
	//
	//if (distance < capsuleR) {
	//	Vector3 collisionNormal = Maths::Vector::Normalise(localP);
...
//bool CollisionDetection::SphereCapsuleIntersection(const CapsuleVolume& volumeA, const Transform& worldTransformA,
	...
	//Vector3 capsuleNormal = Maths::Vector::Normalise(worldTransformA.GetOrientation().ToEuler());
	//
...
Matrix4 CollisionDetection::GenerateInverseView(const Camera &c) {
	...
	Matrix4 iview =
		Maths::Matrix::Translation(position) *
		Maths::Matrix::Rotation(yaw, Vector3(0, 1, 0)) *
		Maths::Matrix::Rotation(pitch, Vector3(1, 0, 0));
