...
UniqueOGLTexture OGLTexture::TextureFromData(char* data, int width, int height, int channels) {
    ...
    switch (channels) {
    case 1: internalFormat = sourceType = GL_RED; break;
    case 2: internalFormat = sourceType = GL_RG; break;
    case 3: sourceType = GL_RGB; internalFormat = GL_SRGB; break;
    case 4: sourceType = GL_RGBA; internalFormat = GL_SRGB_ALPHA; break;
    }

    glBindTexture(GL_TEXTURE_2D, tex->texID);

    glTexImage2D(GL_TEXTURE_2D, 0, internalFormat, width, height, 0, sourceType, GL_UNSIGNED_BYTE, data);
